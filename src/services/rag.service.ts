...
export class RagService {
   ...
   */
  public static async parseFile(filePath: string, fileType: string): Promise<string> {
   ...
   */
  public static splitText(text: string, chunkSize = 600, chunkOverlap = 120): string[] {
    const chunks: string[] = [];
  ...
  public static async searchKnowledgeBase(
    ...
    try {
      const kbDocs = await prisma.knowledgeBase.findMany({
        where: { userId },
      });
      ...
      // Collect all chunks. We'll split the cached document contents.
      interface ChunkItem {
        docId: string;
        fileName: string;
        content: string;
   ...
   */
  private static searchLocalTfIdf(query: string, chunks: { content: string }[], limit: number): string[] {
    ...
    const docScores = chunks.map((chunk, docIndex) => {
      ...
      if (docTokens.length === 0) return { chunk: chunk.content, score: 0 };
   ...
   */
  private static async searchSemanticGemini(
    query: string,
    ...
    if (!queryVector) throw new Error('Could not get query embedding from Gemini');
    ...
    const chunksRes = await axios.post(
    ...
    if (!embeddings || embeddings.length === 0) throw new Error('Could not get chunk embeddings from Gemini');
   ...
   */
  private static async searchSemanticOpenAi(
    query: string,
    chunks: { content: string }[],
    apiKey: string,
    limit: number
    ...
    if (!queryVector) throw new Error('Could not get query embedding from OpenAI');
    ...
    if (!embeddings) throw new Error('Could not get chunk embeddings from OpenAI');
  ...
  private static parseKeys(apiKeysStr: string): Record<string, string> {
    try {
